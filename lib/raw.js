...
!function (exports){
  ...
  raw.parser = function (delimiter) {
    ...
    function ParseError(message) {
      ...
      this.message = message || "Hubo un error al parsear los datos.";
    }
  ...
  raw.model = function () { 
    ...
    model.instruction = function() {
      return dimensions.values()
        .filter(function(d){ return d.required() > d.value.length; })
        .map(function(d){ var v = d.required() - d.value.length > 1 ? 'dimensions' : 'dimension'; return '<b>'+d.title() + "</b> requiere al menos " + (d.required() - d.value.length) + " más " + v;  })
        .join(". ")
    }
